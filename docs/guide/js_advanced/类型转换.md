...
为了将对象转换为相应的基本类型值，抽象操作 ToPrimitive （参考规范 [9.1](https://262.ecma-international.org/5.1/#sec-9.1)）会首先 （通过内部操作 DefaultValue，参考规范 [8.12.8](https://262.ecma-international.org/5.1/#sec-8.12.8)）检查该值是否有 `valueOf()` 方法，如果有并且返回基本类型值，就使用该值进行强制类型转换。如果没有就使用 `toString()` 方法的返回值来进行强制类型转换。
